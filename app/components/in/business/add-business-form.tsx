    ...
    onSuccess: async (data) => {
      ...
      if (data.status === "success") {
        await queryClient.invalidateQueries({
          queryKey: ["auth"],
          refetchType: "active", // ✅ force refetch if query is active
        });
    ...
    <div className="w-full flex flex-col justify-start space-y-4 items-center ">
      ...
      <div className="w-full max-w-2xl mt-6">
        ...
        >
          ...
          />
          <input
            placeholder="Numéro de la patente"
          ...
          >
            ...
            {BUSINESS_DOMAINS_ISIC.map((domain) => (
          ...
          <input
            type="submit"
            value={"Enregistrer"}
            className="w-full bg-black text-white hover:bg-gray-800 py-2 rounded cursor-pointer"
          />
        </form>
      </div>
    </div>
